...
与主项目代码完全独立（`flac-converter/` 有自己的 pom 与 jar），界面为 选输入目录 → 扫描 → 选输出目录 → 开始转换 / 清空列表。

"高级设置"里可选输出摆放方式（选择会记住，下次启动仍是该设置）：
- **平铺（默认）**：所有 MP3 直接放输出目录顶层。不同专辑里的同名歌会往上取目录名区分，
  例如 `Like Home.mp3` 与 `Like Home [Album B].mp3`；连目录都区分不开时才补 `(2)` 序号。
- **保持原目录层级**：按扫描根目录的相对路径重建文件夹，输出同名 `.mp3`。
